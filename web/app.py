"""Backend web autosuficiente: contiene la logica de conversion (copiada de
ireal_def.py, sin la GUI) + servidor Flask. No importa nada del proyecto padre.

POST /convert: sube fuente (iReal url / .musicxml / .txt) -> PDF + MIDI.
  output=full  -> .zip con la carpeta (txt + fuente + pdf + mid)
  output=pdf   -> solo el .pdf
  output=midi  -> solo el .mid
"""
import io
import zipfile
import tempfile
...
ALLOWED_EXT = {".musicxml", ".xml", ".txt"}


@app.after_request
def add_cors(resp):
    """CORS: el frontend (GitHub Pages) vive en otro dominio y llama a este backend.
    Expose-Headers permite que el JS lea el nombre del archivo y los avisos."""
    resp.headers["Access-Control-Allow-Origin"] = "*"
    resp.headers["Access-Control-Allow-Methods"] = "POST, OPTIONS"
    resp.headers["Access-Control-Allow-Headers"] = "*"
    resp.headers["Access-Control-Expose-Headers"] = "Content-Disposition, X-Warnings"
    return resp


def _zip_dir(root):
    """Comprime todo el contenido de `root` en memoria. Devuelve (bytes, nombre.zip)."""
    folders = [e for e in os.listdir(root) if os.path.isdir(os.path.join(root, e))]
    buf = io.BytesIO()
    ...
    name = folders[0] if len(folders) == 1 else "canciones"
...
def convert():
    if request.method == "OPTIONS":   # preflight CORS
        return ("", 204)
    url = (request.form.get("url") or "").strip().strip('"')
    transpose = (request.form.get("transpose") or "").strip()
    bpm = (request.form.get("bpm") or "").strip()
    output = request.form.get("output", "full")
    if output not in ("full", "pdf", "midi"):
        output = "full"
    ...
    try:
        if upload and upload.filename:
            ...
            upload.save(src)
        elif url.startswith("irealb://") or url.startswith("irealbook://"):
            src = url
        elif url:
            # texto pegado directamente: lo tratamos como contenido de un .txt
            src = os.path.join(tmp, "cancion.txt")
        ...
        try:
            _folders, warnings = process_source(src, dest, transpose, bpm, output)
        except Exception as e:
            return jsonify(error=str(e)), 400

        if output == "full":
            data, name = _zip_dir(dest)
            ...
            if len(files) == 1:
                ...
                mimetype = "application/pdf" if p.lower().endswith(".pdf") else "audio/midi"
            else:
                data, name = _zip_files(files)
                mimetype = "application/zip"
